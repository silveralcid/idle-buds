...
(() => {
    ...
    sidebar.category('Socials', {
        ...
        if (!nativeManager.isSteam && !nativeManager.isNativeApp)
            socialsItems.unshift({ name: 'Patreon', id: 14, link: 'https://www.patreon.com/melvoridle' });
        for (const s of socialsItems) {
            item(s.name, {
                icon: assets.getURI(`assets/media/main/${s.name.toLowerCase()}.svg`),
                name: '',
                link: s.link,
                onRender: ({ nameEl }) => {
                    nameEl.textContent = getLangString(`PAGE_NAME_MISC_${s.id}`);
                },
            });
        }
    });
